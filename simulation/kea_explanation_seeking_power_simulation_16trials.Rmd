...
load("kea_explanation_seeking_sim_N16_16trials.RData")
```

Notes:
DV: peeking response (binary)
Design: 2 conditions (functional / intrinsic), within subject design
8 sessions, 8 trials per condition (increase if needed)
...
n.per.subject <- 16 # observations per subject
n.per.condition <- 8 # observations per subject and condition
n.sessions <- 8
...
#library(combinat)
con_order <- c("fun-int", "fun-int", "int-fun", "int-fun", "fun-int", "fun-int", "int-fun", "int-fun")
#order<-unlist(permn(con_order))
order<-replicate(n.subject, sample(con_order, n.sessions))

start.data<-as.data.frame(order) %>%
  pivot_longer(c("V1":"V16"), names_to="session", values_to="order")%>%
  mutate(session=str_remove(session, "V"))%>%
...
save.image("kea_explanation_seeking_sim_N16_16trials.RData")
```
...
```{r echo=FALSE}
p.con.power_8tr <- ggplot(data = lrt.data2, aes(x= as.factor(int.per),y = proportion.sign.lrt.con, fill=as.factor(fun.per))) +
  ...
  theme_few()+
  ggtitle("8 trials per condition")+
  geom_text(aes(label=round(proportion.sign.lrt.con,2)), position=position_dodge(width=0.9), vjust=-0.25)
  #theme(legend.position="none")
p.con.power_8tr

ggsave(p.con.power_8tr, filename = "graphics/kea_explanation_seeking_power_16trials_16subj.png", scale = 0.7, height = 5, width = 8)
```
