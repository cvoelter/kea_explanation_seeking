...
demo.data<-read.csv(file="data/demo_data.csv")
...
ggsave(p.con.power, filename = "graphics/kea_explanation_seeking_power.png", scale = 0.7, height = 5, width = 8)
```
...
p.legend <- ggplot(data = lrt.data2, aes(x= as.factor(int.per),y = proportion.sign.lrt.con, fill=as.factor(fun.per))) +
  ...
  labs(fill = "Intrinsic condition", y="Power", x= "Strategic condition") +
  theme_few()+
