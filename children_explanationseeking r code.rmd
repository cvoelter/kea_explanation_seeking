...
write.csv(mm1_overall_peeking_researcher_absent_output_table, file = "Saves/Child_mm1_overall_peeking_researcher_absent_S9output_table.csv")
```
...
child_sum_peeking_plot_researcher_absent <-
  ...
  ylim(0, 4) +
  theme_classic()+
  ggtitle("Children - Researcher Absent Period")

child_sum_peeking_plot_researcher_absent
```
```{r}
ggsave(child_sum_peeking_plot_researcher_absent , filename = "Graphics/children_binary_peeking_plot_researcher_absent.png", width = 6, height = 6, scale = 0.7)
...
```{r}
save.image("Child_explanation_seeking_researcherabsent.RData")
```
...
## write pub data
```{r}
pub_data <- peeking_data_agg_trial2_researcher_absent %>%
  select(1:3, peeking_count = peeking_response, 5)%>%
  full_join(peeking_data_proportion_researcher_absent)%>%
  select(-count)

write.csv(pub_data, file = "suppl_data_exp2_children_researcher_absent.csv")
```
